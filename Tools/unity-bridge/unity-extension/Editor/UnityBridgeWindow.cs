...
namespace UnityBridge
{
    ...
    public class UnityBridgeWindow : EditorWindow
    {
        ...
        [MenuItem("Window/General/Unity Bridge")]
        public static void ShowWindowFromGeneral()
        {
            ShowWindow();
        }
        ...
        private string BuildCursorMcpConfigJson()
        {
            ...
            var json =
                ...
                "      \"command\": \"node\",\n" +
                $"      \"args\": [ \"{indexPath}\" ],\n" +
                $"      \"env\": {{ \"NODE_ENV\": \"production\", \"UNITY_BASE_URL\": \"http://localhost:{serverPort}\" }}\n" +
                "    }\n" +
                "  }\n" +
                "}";
            return json;
        ...
        private string GetUnityMcpIndexPath()
        {
            ...
            var localPath = Path.Combine(projectRoot, "unity-mcp", "index.js");
            if (File.Exists(localPath))
                return localPath;

            var siblingPath = Path.GetFullPath(Path.Combine(projectRoot, "..", "unity-mcp-advanced", "unity-mcp", "index.js"));
